<template>
  <div class="h-auto">
    <div class="relative" style="min-height: 200px">
      ...
      >
        <tr
          v-for="(sync, idx) in failedSyncs.data"
          :key="idx"
        ...
        >
          ...
          >
            <span
              class="status-badge status-badge--failed inline-flex rounded-full bg-red-100 px-2 text-xs font-medium leading-5"
            >
              {{ sync.status }}
            </span>
            <UiDropdownMenu
      ...
      <div v-if="!$fetchState.pending && !failedSyncs.meta.total">
        <span
          class="icon icon--tick mbx-2 mt-10 w-full text-center text-3xl text-green-600"
        />
        <p class="block w-full text-center text-2xl">No Failed Syncs</p>
      </div>
      <div
        v-if="!$fetchState.pending && failedSyncs.meta.total"
        class="relative h-20"
      >
        ...
        <p class="absolute top-14 left-0 right-0 text-center">
          ...
          {{ failedSyncs.meta.total }} results
        </p>
      </div>
    </div>
    <Modal id="failedDetailsModal" title="More Details">
...
export default {
  ...
  async fetch() {
    this.failedSyncs = await this.getSyncReports();
    this.$emit('loaded', this.failedSyncs);
  },
  computed: {
    totalPages() {
      let total = 0;
      if (this.failedSyncs) {
        total = this.failedSyncs.meta.last_page;
      }

      return total;
    },
    resultsLoadedCount() {
      if (this.failedSyncs?.data) {
        return this.failedSyncs.data.length;
      }
  ...
  methods: {
    ...
    async getServiceLogs(sync) {
      ...
      this.openModal('failedDetailsModal');
      this.selectedSync.serviceLogs = await this.$axios
        ...
        .catch(() => {
          this.closeModal('failedDetailsModal');
          this.$notify(
            {
              group: 'notifications',
              type: 'error',
    ...
    async getMoreRows() {
      ...
      this.failedSyncs.data.push(...res.data);
    },
    getSyncReports() {
      const params = {
        status: 'failed',
