<template>
  <div>
    <div
      class="items-center py-4 space-y-3 sm:space-y-0 sm:flex"
    >
      <div class="flex-1">
        <label class="mb-1">Event Type</label>
        <select
          v-model="selectedEventType"
          class="content--full-width w-full"
        >
          <option
    ...
    <div
      v-for="field in webhookSchemaFields"
      :key="field.key"
      class="items-center py-4 space-y-3 sm:space-y-0 sm:flex"
    >
      <div class="flex-1">
        ...
        <input
          ...
          :class="{
            'form-item__failed': formHasErrors && getValue(field.key) === null,
          }"
          class="content--full-width w-full bg-white mb-3"
        />
        <TreeSelect
          v-if="
            field.type === 'tree-select' &&
            selectedEventType.schema_values[field.key] !== null
          "
          v-model="formData[field.key]"
...
export default {
  ...
  data() {
    return {
      ...
      webhookURL: process.env.WEBHOOK_HANDLER_URL,
    };
  },
  computed: {
    url() {
      ...
      return `${
        this.webhookURL
      }/${this.service.source_system.factory_name.toLowerCase()}/${method}/webhook/${
        this.service.username
      }/${formattedEventType}/${this.service.destination_system.factory_name.toLowerCase()}/${
        this.service.id
      }`;
    },
    webhookSchemaFields() {
  ...
  methods: {
    ...
    async handleSubmit() {
      ...
      try {
        const webhookResponse = await this.$axios
          .$post('/api/api/v2/webhooks', {
          ...
          this.$notify(
            {
              group: 'notifications',
              type: 'success',
              text: 'Webhook successfully created',
            },
            5000
          );
        }
